...
public class CameraMove : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (_player.isTouchTop)
           transform.position += Vector3.Lerp(_realCam.position, target1, _camSpeed);
           
            //transform.position = Vector2.MoveTowards(_realCam.position, target1, _camSpeed);
        if (_player.isTouchBottom)
            transform.position += Vector3.Lerp(_realCam.position, target2, _camSpeed);
        if (_player.isTouchRight)
            transform.position += Vector3.Lerp(_realCam.position, target3, _camSpeed);
        if (_player.isTouchLeft)
            transform.position += Vector3.Lerp(_realCam.position, target4, _camSpeed);
